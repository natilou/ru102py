...
class FeedDaoRedis(FeedDaoBase, RedisDaoBase):
    ...
    def _insert(
        ...
        global_stream_key = self.key_schema.global_feed_key()
        site_specific_stream_key = self.key_schema.feed_key(meter_reading.site_id)
        meter_reading_serialized = MeterReadingSchema().dump(meter_reading)

        pipeline.xadd(
            global_stream_key,
            fields=meter_reading_serialized,
            maxlen=self.GLOBAL_MAX_FEED_LENGTH,
        )
        pipeline.xadd(
            site_specific_stream_key,
            fields=meter_reading_serialized,
